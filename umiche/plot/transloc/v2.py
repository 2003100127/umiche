...
class protTrimerSplit(Config.config):
    ...
    def errorband(self, ):
        ...
        ax.set_ylabel('Number of molecules to be \n estimated as artifactual/real', fontsize=12)
    ...
    def sglband(self, norms, mets):
        ...
        for i, met in enumerate(mets):
            ...
            ax.errorbar(
                x=df_met.index,
                y=df_met['mean']/norms[i],
                yerr=[df_met['mean-min']/norms[i], df_met['max-mean']/norms[i]],
                fmt='o',
                alpha=0.65,
                ecolor=cc[i],
                color=cc[i],
            ...
            ax.plot(df_met.index, df_met['mean']/norms[i] - df_met['mean-min']/norms[i], color=cc[i], linewidth=0.1, alpha=0.1)
            ax.plot(df_met.index, df_met['max-mean']/norms[i] + df_met['mean']/norms[i], color=cc[i], linewidth=0.1, alpha=0.1)
            ax.fill_between(
                df_met.index,
                df_met['mean']/norms[i] - df_met['mean-min']/norms[i],
                df_met['max-mean']/norms[i] + df_met['mean']/norms[i],
                alpha=0.1,
                color=cc[i],
            )
            ax.set_xticks(df_met['metric'].index)
            ax.set_xticklabels(df_met['metric'], fontsize=8, rotation=30)

        ax.set_xlabel('Sequencing error rate', fontsize=12)
        ax.set_ylim(0, 1.1)
        ax.set_ylabel('Correctly estimated (%)', fontsize=12)
        # ax.set_ylabel('Number of molecules to be \n estimated as artifactual/real', fontsize=12)
        # ax.set_ylabel(r'$\frac{N_e-N_t}{N_t}$', fontsize=14)
        # ax.set_title(DEFINE['title'], fontsize=12)
        sns.despine(right=True, top=True)
        ...
        plt.legend(fontsize=11)
        plt.savefig('C:/Users/jianf/Desktop/path.eps', format='eps', transparent=True)
        plt.show()
        return
...
if __name__ == "__main__":
    ...
    DEFINE = {
        'fpns': {
            'artifactual (ground truth)': to('data/simu/transloc/trimer/single_read/') + metric_char + '/act_fake_healing_sgl.txt',
            'artifactual': to('data/simu/transloc/trimer/single_read/') + metric_char + '/est_fake_healing_sgl.txt',

            'real (ground truth)': to('data/simu/transloc/trimer/single_read/') + metric_char + '/act_real_healing_sgl.txt',
            'real': to('data/simu/transloc/trimer/single_read/') + metric_char + '/est_real_healing_sgl.txt',
