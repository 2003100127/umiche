...
from umiche.deduplicate.method.umicountr.UMICountR import UMICountR
...
class SpikeUMI:
    ...
    def extract_spike_dat(
        ...
        if match_seq_before_UMI and match_seq_after_UMI:
            ...
            for s in self.console._tqdm(
                    df["seq"].tolist(),
                    # total=bam_in.count(until_eof=True),
                    desc="[spUMI Extraction: Given]",
                    unit="reads",
                    position=0,
                    leave=True,
                    dynamic_ncols=False,
            ...
            for s in self.console._tqdm(
                    df["seq"].tolist(),
                    # total=bam_in.count(until_eof=True),
                    desc="[spUMI Extraction: Not Given]",
                    unit="reads",
                    position=0,
                    leave=True,
                    dynamic_ncols=False,
        ...
        def _corr(group, ed):
            return UMICountR().corrected_umi(group["spikeUMI"].tolist(), ed_thres=ed, collapse_mode="adjacency")

        df["spikeUMI_hd1"] = (
            df.groupby("BC", group_keys=False)["spikeUMI"]
            .apply(lambda s: pd.Series(UMICountR().corrected_umi(s.tolist(), ed_thres=1, collapse_mode="adjacency")))
            .values
        )
        df["spikeUMI_hd2"] = (
            df.groupby("BC", group_keys=False)["spikeUMI"]
            .apply(lambda s: pd.Series(UMICountR().corrected_umi(s.tolist(), ed_thres=2, collapse_mode="adjacency")))
            .values
        )
    ...
    def subsample_recompute(
        ...
        then recompute UB (library UMI) HD1/HD2 UMICountR() corrected_umi within each BC.
        ...
        for bc, sub in df.groupby("BC"):
            ...
            ub_hd1 = UMICountR().corrected_umi(sub2["UX"].tolist(), ed_thres=1, collapse_mode="adjacency")
            ub_hd2 = UMICountR().corrected_umi(sub2["UX"].tolist(), ed_thres=2, collapse_mode="adjacency")
            sub2["UB_hd1"] = ub_hd1
            sub2["UB_hd2"] = ub_hd2
...
if __name__ == "__main__":
    ...
    df_sp = SpikeUMI(verbose=True).extract_spike_dat(
        df=df,
    ...
    # overrep = UMICountR.get_overrepresented_spikes(spikedat, readcutoff=75)
    ...
    for key in keys:
        ...
        print(key, df_sub['UB_directional'].unique().shape[0])

    # 4) subsample
    # sub = UMICountR.subsample_recompute(df_sp, mu_nSpikeUMI=100)
    # print(sub)