...
from umiche.deduplicate.monomer.pipeline import Config
from umiche.deduplicate.monomer.Relation import relation as umimonorel
from umiche.deduplicate.monomer.Trace import trace as umimonotrace
from umiche.deduplicate.monomer.DedupPos import dedupPos
from umiche.deduplicate.monomer.old.Cluster import cluster as umimonoclust
from umiche.deduplicate.monomer.old.Adjacency import adjacency as umitoolmonoadj
from umiche.deduplicate.monomer.old.Directional import directional as umitoolmonodirec
from umiche.deduplicate.monomer.old.MarkovClustering import markovClustering as umimonomcl
from umiche.deduplicate.monomer.DBSCAN import dbscan as dbsc
...
class umi(Config.config):
    ...
    def evaluate(self, ):
        ...
        for id, i_metric in enumerate(self.metrics[self.metric]):
            ...
            ccs_stime = time.time()
            ccs = umimonoclust().cc(graph_adj=umiche.graph_adj)
            ccs_net_num = len([*ccs])
